-- Link species to parks using EXISTING species_parks schema
-- Works with: species_id, park_id, confirmed_sighting, observation_count
...
-- Function 2: Populate species_parks for all parks (using existing columns)
CREATE OR REPLACE FUNCTION populate_species_park_links_simple()
RETURNS TABLE (
    park_name TEXT,
...
BEGIN
    FOR park_rec IN
        ...
        LIMIT 100
    LOOP
        -- Insert matches using existing columns
        INSERT INTO species_parks (species_id, park_id, confirmed_sighting, observation_count)
        SELECT
            m.species_id,
            park_rec.id,
            m.match_count > 2,  -- Consider "confirmed" if multiple sample points match
            m.match_count
