...
import { isEcoRegionCompleted, markFoodWebTriviaComplete, markPixelGameComplete, markWhackAMoleComplete, clearEcoRegionProgress } from '@/utils/ecoRegionProgress';
import { CHARACTER_DIALOGUES, getRandomDialogue, GAME_CHARACTERS, getCharacterName } from '@/config/gameCharacters';
import { WhackAMoleGameModal } from '@/components/WhackAMoleGameModal';
import { createPixelGameConfig, type PixelGameConfig } from '@/config/pixelGameMapping';
...
const Index = () => {
  ...
  const [hasBooted, setHasBooted] = useState<boolean>(() => {
    // Check if terminal has booted in this session
    return sessionStorage.getItem('wildlifeTerminalBooted') === 'true';
  });
  const [bootLoadingStatus, setBootLoadingStatus] = useState<string>(''); // Current boot loading message
  ...
  const [minionCounter, setMinionCounter] = useState(1); // Track which minion number to display (1, 2, or 3)

  // 🎮 NEW: Pixel game state
  const [showPixelGame, setShowPixelGame] = useState(false);
  const [pixelGameConfig, setPixelGameConfig] = useState<PixelGameConfig | null>(null);

  // 🎯 NEW: Whack-A-Mole game state
  const [showWhackAMole, setShowWhackAMole] = useState(false);
  const [whackAMoleConfig, setWhackAMoleConfig] = useState<{
    animalType: string;
    animalName: string;
    biomeType: string;
    ecoRegionId: string;
  ...
  // 🎮 Handle return from Pixel Game page
  useEffect(() => {
    ...
    if (pixelGameWon === 'true' && returnedEcoRegionId) {
      ...
      // Set whackAMole config and launch game
      if (pixelGameConfig) {
        setWhackAMoleConfig({
          animalType: pixelGameConfig.animalType,
          animalName: pixelGameConfig.animalName,
          biomeType: pixelGameConfig.biomeType,
          ecoRegionId: returnedEcoRegionId,
        });
      }

      setTimeout(() => {
        setShowWhackAMole(true);
        console.log('🎯 Launching Whack-A-Mole after Pixel Game win');
      }, 500);

      toast({
        title: "🎯 Final Challenge!",
        description: "Battle the invaders to protect the habitat!",
      });
      ...
      toast({
        title: "🔄 Starting Over",
        description: "Game lost! All progress has been reset.",
        variant: "destructive"
      });
    }
  }, [searchParams, setSearchParams, setShowWhackAMole, setWhackAMoleConfig, pixelGameConfig, toast]);

  // 🌍 Load WWF ecoregions from database when terminal boots
  useEffect(() => {
    // Only load eco-regions when the terminal is booted
    if (!hasBooted) return;
    ...
    const loadEcoRegions = async () => {
      try {
        ...
        if (ecoregions) {
          // Convert database ecoregions to pin format with conditional emoji/sprite
          console.log('📍 Creating pins from', ecoregions.length, 'ecoregions, hasBooted:', hasBooted);
          const pins = ecoregions.map(eco => {
            const isCompleted = isEcoRegionCompleted(eco.id);
            const isArctic = eco.name.toLowerCase().includes('arctic');
            const isCongo = eco.name.toLowerCase().includes('congo');
            const isAmazon = eco.name.toLowerCase().includes('amazon');
            const isMadagascar = eco.name.toLowerCase().includes('madagascar');
            const isCoral = eco.name.toLowerCase().includes('coral');
            const isBorneo = eco.name.toLowerCase().includes('borneo');

            // Debug: Log Coral Triangle detection
            if (isCoral) {
              console.log('🐟 Coral Triangle detected:', eco.name);
            }

            // Use sprite for Arctic - Polar Bear
            if (isArctic) {
              return {
                lat: eco.center_lat,
                lng: eco.center_lng,
                ...
                spriteSheet: '/images/polar_bear_walk.png',
                spriteFrames: 8, // 8 frames per direction
                spriteRows: 4,
                spriteCols: 8,
                spriteRow: 0, // Row 0 is walk_down (front-facing)
                walkInCircle: true, // Cycle through all 4 directions
                ecoRegionId: eco.id,
              };
            }

            // Use sprite for Congo Basin - Gorilla Climbing
            if (isCongo) {
              return {
                lat: eco.center_lat,
                lng: eco.center_lng,
                ...
                color: eco.realm === 'Marine' ? '#3b82f6' : '#22c55e',
                type: 'habitat' as const,
                spriteSheet: '/images/gorilla_climb.png',
                spriteFrames: 5, // 5 frames per row (10 frames total)
                spriteRows: 2,
                spriteCols: 5,
                spriteRow: 0, // Start at row 0
                walkInCircle: true, // Cycle through all rows
                ecoRegionId: eco.id,
              };
            }

            // Use sprite for Amazon - Parrot Flying
            if (isAmazon) {
              return {
                lat: eco.center_lat,
                lng: eco.center_lng,
                ...
                color: eco.realm === 'Marine' ? '#3b82f6' : '#22c55e',
                type: 'habitat' as const,
                spriteSheet: '/images/parrot_flying.png',
                spriteFrames: 4, // 4 frames per row
                spriteRows: 4,
                spriteCols: 4,
                spriteRow: 0, // Row 0 for flying animation
                walkInCircle: true, // Cycle through all 4 rows
                ecoRegionId: eco.id,
              };
            }

            // Use sprite for Madagascar - Chameleon
            if (isMadagascar) {
              return {
                lat: eco.center_lat,
                lng: eco.center_lng,
                ...
                color: eco.realm === 'Marine' ? '#3b82f6' : '#22c55e',
                type: 'habitat' as const,
                spriteSheet: '/images/chameleon_idle.png',
                spriteFrames: 5, // 5 frames per row
                spriteRows: 4,
                spriteCols: 5,
                spriteRow: 0, // Start at row 0
                walkInCircle: true, // Cycle through all 4 rows (20 frames total)
                ecoRegionId: eco.id,
              };
            }

            // Use realistic photo for Coral Triangle - Lionfish
            if (isCoral) {
              const coralPin = {
                lat: eco.center_lat,
                lng: eco.center_lng,
                ...
                color: eco.realm === 'Marine' ? '#3b82f6' : '#22c55e',
                type: 'habitat' as const,
                imageUrl: '/images/lionfish_photo.jpg',
                ecoRegionId: eco.id,
              };
              console.log('🐟 Coral Triangle pin object:', coralPin);
              return coralPin;
            }

            // Use sprite for Borneo - Elephant
            if (isBorneo) {
              return {
                lat: eco.center_lat,
                lng: eco.center_lng,
                ...
                color: eco.realm === 'Marine' ? '#3b82f6' : '#22c55e',
                type: 'habitat' as const,
                spriteSheet: '/images/elephant_walk.png',
                spriteFrames: 3, // 3 frames per direction
                spriteRows: 4,
                spriteCols: 3,
                spriteRow: 2, // Row 2 is South/Down (front-facing)
                walkInCircle: true, // Cycle through all 4 directions
                ecoRegionId: eco.id,
              };
            }
          ...
          console.log('📍 Eco-region pins:', pins);
        }
      } catch (err) {
...
${randomSpecies.conservationStatus ? `**Status:** ${randomSpecies.conservationStatus}` : ''}`;
        ...
        // 🤖 Generate Guardian AI dialogue with robotic personality
        const guardianDialogue = getRandomDialogue(
          CHARACTER_DIALOGUES.guardian.regionEntry,
          { regionName: regionInfo.regionName }
        );
        const fullGreeting = guardianDialogue;
...
${expandedStatus ? `Status: ${expandedStatus}` : ''}`;
        ...
        setTimeout(() => {
          ...
          const charSheetInterval = setInterval(() => {
            if (charSheetIndex < characterSheetText.length) {
              ...
              // Add empty greeting message with Guardian character info
  ...
  */
  ...
  const handleEcoRegionClick = async (point: any) => {
    ...
    // 🌍 Fetch eco-region details from database
    try {
      const { supabase } = await import('@/integrations/supabase/client');
      const { data: ecoregion, error } = await supabase
        .from('ecoregions')
        .select('*')
        .eq('id', point.ecoRegionId)
        .single();

      if (error) {
        console.error('Error fetching eco-region:', error);
        toast({
          title: "Error",
      ...
      // 🎨 Generate dynamic color theme based on eco-region (chameleon effect!)
      console.log('🎨 Generating chameleon theme for:', ecoregion.name, 'Biome:', ecoregion.biome);
      const newTheme = await generateDynamicTheme(ecoregion.name, ecoregion.biome);
      setChatTheme(newTheme);
      console.log('✅ Chameleon theme applied:', newTheme);

      // 📍 Navigate directly to park selection page
      console.log('✅ Entering eco-region:', ecoregion.name);
      navigate(`/park-select?ecoRegionId=${ecoregion.id}&regionName=${encodeURIComponent(ecoregion.name)}&lat=${point.lat}&lng=${point.lng}`);
    } catch (err) {
      console.error('Failed to fetch eco-region:', err);
      toast({
        title: "Error",
        description: "Failed to load eco-region data",
        variant: "destructive"
  ...
  const handleQuickReply = (reply: QuickReply) => {
    ...
    switch (reply.action) {
      ...
      case 'play-food-web-game':
        // 🤖 Navigate to Trivia Page for mini-game flow
        setQuickReplies([]); // Clear the button

        if (regionInfo && currentEcoRegionId) {
          // Navigate to trivia page with chat history and state
          navigate('/trivia', {
            state: {
              ecoRegionId: currentEcoRegionId,
              regionName: regionInfo.regionName,
              chatHistory: chatHistory,
              selectedFoodWebSpecies: selectedFoodWebSpecies,
            }
          });

          console.log('🤖 Navigating to Trivia Page with chat history');
        } else {
          toast({
            title: "⚠️ Cannot Start Game",
            description: "Please complete the Food Web Trivia first!",
            variant: "destructive"
          });
        }
        return; // Don't send a message
  ...
  // 🎮 2D PIXEL GAME COMPLETION HANDLER - Launch Whack-A-Mole as final game!
  const handlePixelGameComplete = async (ecoRegionId: string) => {
    markPixelGameComplete(ecoRegionId);
    console.log('💾 Saved 2D Pixel Game completion for eco-region:', ecoRegionId);

    // Get config for Whack-A-Mole (reuse pixel game config)
    if (pixelGameConfig) {
      setWhackAMoleConfig({
        animalType: pixelGameConfig.animalType,
        animalName: pixelGameConfig.animalName,
      ...
      // Show Whack-A-Mole game after a brief delay
      setTimeout(() => {
        setShowWhackAMole(true);
        console.log('🎯 Launching Whack-A-Mole as final game!');
      }, 1500);
  ...
  // 🎯 WHACK-A-MOLE WIN HANDLER - Complete the entire eco-region!
  const handleWhackAMoleWin = async (ecoRegionId: string) => {
    markWhackAMoleComplete(ecoRegionId);
    console.log('💾 Saved Whack-A-Mole completion - ECO-REGION FULLY COMPLETED!');
    ...
    if (ecoregions) {
      ...
      setEcoRegionPins(pins);
      console.log('🔄 Reloaded eco-region pins - region fully completed!');
    }

    toast({
      title: "🎉🎉🎉 ECO-REGION COMPLETE! 🎉🎉🎉",
      description: "You've cleaned up this eco-region! All three games completed!",
    });
  };

  // 🔄 WHACK-A-MOLE LOSS HANDLER - Reset entire eco-region flow
  const handleWhackAMoleLose = (ecoRegionId: string) => {
    clearEcoRegionProgress(ecoRegionId);
    console.log('🔄 Whack-A-Mole lost - clearing ALL progress for eco-region:', ecoRegionId);

    // Clear selected food web species
    ...
    // Clear chat history to restart conversation
    setChatHistory([]);

    toast({
      title: "🔄 Starting Over",
      description: "Time's up! All progress for this eco-region has been reset.",
      variant: "destructive"
    });
  };

  // 🔄 PIXEL GAME LOSS HANDLER - Reset entire eco-region flow
  const handlePixelGameLose = (ecoRegionId: string) => {
    clearEcoRegionProgress(ecoRegionId);
    console.log('🔄 Game lost - clearing all progress for eco-region:', ecoRegionId);
    ...
    toast({
      title: "🔄 Starting Over",
      description: "Game lost! All progress for this eco-region has been reset.",
      variant: "destructive"
    });
  };
  ...
  const handleSearch = async (query: string) => {
    ...
    if (query.trim().startsWith('/')) {
      ...
      // 🚫 Block all commands except /start and /reset until terminal is booted
      if (!hasBooted && command !== '/start' && command !== '/reset') {
        const blockedMessage: ChatMessage = {
          id: `blocked-${Date.now()}`,
          role: 'assistant',
          content: 'Wildlife Terminal is offline. Please type /start to begin.',
          timestamp: new Date(),
          status: 'sent'
        };
      ...
      // /start command - Boot the Wildlife Terminal
      if (command === '/start') {
        if (hasBooted) {
          const alreadyBootedMessage: ChatMessage = {
            id: `already-booted-${Date.now()}`,
            role: 'assistant',
            content: 'Wildlife Terminal is already online. Type /help for available commands.',
            timestamp: new Date(),
            status: 'sent'
          };
          setChatHistory(prev => [...prev, alreadyBootedMessage]);
          setIsChatHistoryExpanded(true);
          setIsLoading(false);
          return;
        }
        ...
        const loadingSteps = [
          ...
          'Syncing conservation APIs...',
          'Loading trivia games...',
          'Initializing wildlife database...',
  ...
  <h1 style="color: #10b981; margin: 0;">CRITTER AI</h1>
        ...
        const loadingInterval = setInterval(() => {
          ...
          if (loadingStepIndex >= loadingSteps.length) {
            // All loading steps complete - go online
            clearInterval(loadingInterval);
            setBootLoadingStatus('');
            setHasBooted(true);
            sessionStorage.setItem('wildlifeTerminalBooted', 'true');
            return;
          }
...
${currentText}`,
  ...
  // /enter command - Enter selected eco-region
  if (command === '/enter') {
        if (!selectedEcoRegion) {
          const noRegionMessage: ChatMessage = {
            id: `no-region-${Date.now()}`,
            role: 'assistant',
            content: 'No eco-region selected. Click on a region on the globe first.',
            timestamp: new Date(),
            status: 'sent'
          };
          setChatHistory(prev => [...prev, noRegionMessage]);
          setIsChatHistoryExpanded(true);
          setIsLoading(false);
          return;
        }

        // Navigate to park selection page for the selected eco-region
        navigate(`/park-select?ecoRegionId=${selectedEcoRegion.id}&regionName=${encodeURIComponent(selectedEcoRegion.name)}&lat=${selectedEcoRegion.lat}&lng=${selectedEcoRegion.lng}`);
        setIsLoading(false);
        return;
      }

      // /cancel command - Cancel eco-region selection
      if (command === '/cancel') {
        setSelectedEcoRegion(null);

        // 🎨 Reset theme to default emerald (chameleon returns to default state)
        setChatTheme({
          primary: 'hsl(160, 84%, 39%)',
          secondary: 'hsl(158, 64%, 52%)',
          background: 'hsl(222, 47%, 11%)',
        ...
        console.log('🎨 Reset theme to default emerald');

        const cancelMessage: ChatMessage = {
          id: `cancel-${Date.now()}`,
          role: 'assistant',
          content: 'Selection cancelled. Click another region to explore.',
          timestamp: new Date(),
          status: 'sent'
        };
      ...
      // /learn command - Learn more about selected eco-region
      if (command === '/learn') {
        if (!selectedEcoRegion) {
          const noRegionMessage: ChatMessage = {
            id: `no-region-learn-${Date.now()}`,
            role: 'assistant',
            content: 'No eco-region selected. Click on a region on the globe first.',
            timestamp: new Date(),
            status: 'sent'
        ...
        // Add educational content about the eco-region
        const learnMessage: ChatMessage = {
          id: `learn-${Date.now()}`,
          role: 'assistant',
          content: `📚 Learning about **${selectedEcoRegion.name}**\n\nThis feature will provide detailed information about the eco-region's:\n- Ecosystem characteristics\n- Biodiversity statistics\n- Conservation threats\n- Protected areas\n- Active organizations\n\n(Educational content system in development)`,
          timestamp: new Date(),
          status: 'sent'
        };
      ...
      // /reset command - Reset terminal to offline state (for testing /start)
      if (command === '/reset') {
        // Clear boot state
        sessionStorage.removeItem('wildlifeTerminalBooted');
        setHasBooted(false);

        // Clear eco-region pins
        setEcoRegionPins([]);

        // Clear selected eco-region
        setSelectedEcoRegion(null);
        ...
        const resetMessage: ChatMessage = {
          id: `reset-${Date.now()}`,
          role: 'assistant',
          content: '🔄 Wildlife Terminal has been reset to offline state.\n\nType /start to boot up again.',
          timestamp: new Date(),
          status: 'sent'
        };
        ...
        toast({
          ...
          description: 'Wildlife Terminal is now offline. Type /start to begin.',
        });
    ...
    // 🚫 Block all non-command queries until terminal is booted
    if (!hasBooted) {
      const blockedMessage: ChatMessage = {
        id: `blocked-${Date.now()}`,
        role: 'assistant',
  ...
  const handleCarouselSpeciesSelect = async (species: RegionSpecies, learningTopic?: string) => {
    ...
    if (!isFoodWebGameActive) {
      ...
      if (species.imageUrl) {
        console.log('🖼️ Adding image to chat history');
        const imageMessage: ChatMessage = {
          id: `species-image-${Date.now()}`,
          role: 'assistant',
          content: species.imageUrl,
          timestamp: new Date(),
          status: 'sent',
          isImage: true
        };
        ...
        // Wait for image to load AND let loading animation finish before streaming text
        const img = new Image();
        img.onload = () => {
          console.log('✅ Image loaded, waiting 7 seconds before showing character sheet...');
          // Wait 7 seconds for laser loading effect to complete and user to view the image
          setTimeout(() => {
            console.log('✅ Loading animation complete, starting character sheet stream');
            streamCharacterSheet();
          }, 7000);
        };
        img.onerror = () => {
          console.log('❌ Image failed to load, streaming character sheet anyway');
          setTimeout(() => streamCharacterSheet(), 7000);
        };
        img.src = species.imageUrl;
      } else {
        // No image, stream immediately
        streamCharacterSheet();
      }

      // 2️⃣ Stream species character sheet function
      const streamCharacterSheet = () => {
        console.log('📝 streamCharacterSheet called - creating text message now');
        const characterSheet = `**${species.commonName}** (*${species.scientificName}*)

**Conservation Status:** ${species.conservationStatus || 'Unknown'}
**Dietary Category:** ${species.dietaryCategory || 'Unknown'}
**Animal Type:** ${species.animalType || 'Unknown'}
**Occurrences in ${regionInfo?.regionName || 'this region'}:** ${species.occurrenceCount || 0}`;

        // Create streaming message
        const assistantMessageId = `species-info-${Date.now()}`;
        const assistantMessage: ChatMessage = {
          id: assistantMessageId,
          role: 'assistant',
          content: '',
        ...
        console.log('➕ Adding text message to chat history');
        setChatHistory(prev => [...prev, assistantMessage]);

        // Stream character-by-character (30ms = slow, comfortable reading speed)
        setIsLoading(true);
        let charIndex = 0;
        const streamInterval = setInterval(() => {
          if (charIndex >= characterSheet.length) {
            clearInterval(streamInterval);
            setIsLoading(false);
          ...
          const nextChar = characterSheet[charIndex];
          charIndex++;
        ...
        }, 30); // 30ms per character (slow, readable streaming)
      };
    ...
    if (validation.correct) {
      ...
      if (foodWebGamePhase >= 3) {
        ...
        if (currentEcoRegionId) {
          ...
          // 🎮 Launch Pixel Game!
          // Use the last found species as the player character
          if (selectedSpeciesForReveal && regionInfo) {
            const gameConfig = createPixelGameConfig(
              selectedSpeciesForReveal,
              currentEcoRegionId,
            ...
            // Wait 2 seconds before navigating (let celebration message show first)
            setTimeout(() => {
              const params = new URLSearchParams({
                ecoRegionId: gameConfig.ecoRegionId,
                animalType: gameConfig.animalType,
                animalName: gameConfig.animalName,
                biomeType: gameConfig.biomeType,
              });
              navigate(`/pixel-game?${params.toString()}`);
            }, 2000);

            console.log('🎮 Navigating to Pixel Game page:', gameConfig);
          }
        }
        ...
        toast({
          title: "🎉 Game Complete!",
          description: "You found all 5 species! Get ready for the next challenge...",
        });
      } else {
        // Move to next phase
...
${question.choices.join('\n')}`;
  ...
  const streamTextToChat = async (
    ...
    characterEmoji?: string, // Optional character emoji
    ...
    if (isCorrect) {
      ...
      setTimeout(() => {
        ...
        if (newCollectedCount >= 3) {
          ...
          // Add "Play Poopy Minion Escape" button in chat
          setQuickReplies([
            { id: 'play-food-web-game', label: 'Play Poopy Minion Escape', emoji: '🤖', action: 'play-food-web-game' as const }
          ]);
        } else {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-background">
        ...
        {useGoogleMaps ? (
          ...
          <GlobeComponent
            ...
            onImageMarkerClick={(point) => {
              // Check if this is an ecoregion pin (has ecoRegionId property)
              console.log('📍 onImageMarkerClick called:', point.name, 'ecoRegionId:', point.ecoRegionId);
              if (point.ecoRegionId) {
                console.log('✅ Routing to handleEcoRegionClick');
                handleEcoRegionClick(point);
              } else {
                console.log('➡️ Routing to handleImageMarkerClick');
      ...
      {/* Priority 0: Selected Eco-Region from Globe Click (HIGHEST PRIORITY) */}
      {selectedEcoRegion && !useGoogleMaps ? (
        <div
          className="absolute right-6 w-80 z-[60] pointer-events-auto flex flex-col gap-3"
          style={{ top: '90px', maxHeight: 'calc(100vh - 200px)', overflow: 'auto' }}
        >
          <InfoCard
            type="ecoregion"
            regionName={selectedEcoRegion.name}
            description={selectedEcoRegion.description}
            speciesCount={selectedEcoRegion.speciesCount || 0}
            locationCount={selectedEcoRegion.locationCount || 0}
            imageUrl={selectedEcoRegion.imageUrl}
          />
          
          {/* Terminal Command Prompts */}
          <div className="glass-panel rounded-xl p-4 border border-primary/30">
            <p className="text-sm text-muted-foreground mb-3">Available commands:</p>
            <div className="flex flex-col gap-2">
              <button
                onClick={() => handleSearch('/enter')}
                className="glass-panel hover:bg-primary/20 text-primary border border-primary/40 rounded-lg px-4 py-2 text-sm font-medium transition-all"
              >
                /enter - Enter this eco-region
              </button>
              <button
                onClick={() => handleSearch('/learn')}
                className="glass-panel hover:bg-accent/20 text-accent border border-accent/40 rounded-lg px-4 py-2 text-sm font-medium transition-all"
              >
                /learn - Learn more about this region
              </button>
              <button
                onClick={() => handleSearch('/cancel')}
                className="glass-panel hover:bg-destructive/20 text-destructive border border-destructive/40 rounded-lg px-4 py-2 text-sm font-medium transition-all"
              >
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 z-30 w-full max-w-[1250px] flex flex-col items-center gap-3 pointer-events-none pb-2">
        ...
        {/* Terminal UI - Always show */}
        <div className="flex justify-center items-end gap-3 w-full pointer-events-auto">
          <div className={`w-full flex flex-col ${useGoogleMaps ? 'max-w-[900px]' : 'max-w-[650px]'}`}>
              ...
              {chatHistory.length > 0 && (
                <ChatHistory
                  ...
                  theme={chatTheme}
                  hasBooted={hasBooted}
                  bootLoadingStatus={bootLoadingStatus}
                />
              )}

              {/* Terminal Header - Show only when offline AND no messages yet */}
              {!useGoogleMaps && !hasBooted && chatHistory.length === 0 && (
      ...
      {whackAMoleConfig && (
        <WhackAMoleGameModal
          ...
          animalType={whackAMoleConfig.animalType}
          animalName={whackAMoleConfig.animalName}
          biomeType={whackAMoleConfig.biomeType}
          ecoRegionId={whackAMoleConfig.ecoRegionId}
          onGameComplete={handleWhackAMoleWin}
          onGameLose={handleWhackAMoleLose}
        />
      )}
